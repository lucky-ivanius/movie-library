...
const feature = loadFeature(path.join(__dirname, './deleteAuthor.feature'));
...
defineFeature(feature, (test) => {
  ...
  test('Delete author with valid identity', ({ given, when, then }) => {
    given('a valid author identity', () => {
      id = dummyAuthor.id.toString();
    });

    when('attempt to delete author', async () => {
      useCaseResult = await deleteAuthorUseCase.execute({
        id
      });
    });

    then('author should be deleted successfully', async () => {
      ...
      const deletedAuthor = await authorRepo.findAuthorById(id);
      expect(deletedAuthor).toBeNull();
    });
  });

  test('Delete author with invalid identity format', ({
    given,
    when,
    then
  }) => {
  ...
  test('Delete author with invalid identity', ({ given, when, then }) => {
